...
export class FfmpegProcessSupervisor {
  ...
  private terminalListeners = new Set<(outcome: TerminalOutcome) => void>();
  ...
  onTerminal(listener: (outcome: TerminalOutcome) => void): () => void {
  ...
  private publish(type: string, payload: Record<string, unknown>): void {
    const eventType = this.options.eventPrefix
  ...
  private terminal(outcome: TerminalOutcome): void {
    for (const listener of this.terminalListeners) listener(outcome);
  }
  ...
  start(command: FfmpegCommand): void {
    ...
    if (state !== "idle" && state !== "failed")
      throw new Error(`Pipeline cannot start from ${state}`);
    ...
    child.once("spawn", () => {
      if (!child.pid)
        return this.fail(new Error("FFmpeg started without a process id"));
      this.lifecycle.attachProcess(child.pid);
      this.publish("started", {
        pid: child.pid,
        outputs: command.outputCount,
        args: command.redactedArgs,
      });
    });
    ...
    child.stderr.on("data", (chunk: string) => {
      ...
      if (message) this.publish("stderr", { message: message.slice(-2000) });
    });
    ...
    child.once("exit", (code, signal) => {
      ...
      if (this.stopping && this.lifecycle.getSnapshot().state === "stopping") {
        ...
        this.publish("stopped", { code, signal });
        this.terminal({ kind: "stopped" });
      } else if (code !== 0) {
        this.fail(
          new Error(
            `FFmpeg exited with code ${code ?? "null"} (${signal ?? "no signal"})`,
          ),
        );
      } else {
        const current = this.lifecycle.getSnapshot().state;
        if (current === "running") this.lifecycle.transition("stopping");
        if (this.lifecycle.getSnapshot().state === "stopping")
          this.lifecycle.transition("idle");
        this.publish("completed", { code, signal });
  ...
  async stop(graceMs = 8000): Promise<void> {
    ...
    if (!child) {
      if (this.lifecycle.getSnapshot().state === "failed")
        this.lifecycle.reset();
      return;
    }
    const state = this.lifecycle.getSnapshot().state;
  ...
  private parseProgress(line: string): void {
    ...
    if (key === "bitrate")
      this.metrics.bitrateKbps =
        Number.parseFloat(raw.replace("kbits/s", "")) || 0;
    if (key === "speed")
      this.metrics.speed = Number.parseFloat(raw.replace("x", "")) || 0;
    if (key === "out_time_ms" && Number.isFinite(number))
      this.metrics.outTimeMs = number;
    if (key === "progress") {
      this.lastProgressAt = new Date().toISOString();
      this.publish("metrics", { ...this.metrics });
    }
  }
  ...
  private fail(error: unknown): void {
    try {
      ...
      /* lifecycle already terminal */
    }
    const message = error instanceof Error ? error.message : String(error);
    this.publish("failed", { error: message });
    this.terminal({ kind: "failed", error: message });
  }
}
