...
export type DestinationStatus = 'idle' | 'configured' | 'armed' | 'connecting' | 'live' | 'degraded' | 'failed' | 'cooldown' | 'disabled'
export type StreamStatus = 'online' | 'reconnecting' | 'offline' | 'standby'
export type PipelineHealth = 'ok' | 'degraded' | 'failed'
export type Provider = 'youtube' | 'telegram' | 'tiktok' | 'twitch' | 'instagram' | 'mixer' | 'mixcloud' | 'facebook' | 'custom-rtmp' | 'local'

export interface LayoutState { mode: UiMode }
...
export interface SceneGraph { width: number; height: number; background: string; layers: SceneLayer[] }
...
export interface TransmissionChecklistItem { id: string; label: string; required: boolean; completed: boolean }
