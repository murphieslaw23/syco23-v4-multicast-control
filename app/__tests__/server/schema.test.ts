...
describe('server/schema', () => {
  ...
  it('includes transmission_kits table', () => {
    expect(SCHEMA_SQL).toContain('CREATE TABLE IF NOT EXISTS transmission_kits')
  })
  ...
  it('detects missing tables', () => {
    const incomplete = 'CREATE TABLE IF NOT EXISTS streams (id TEXT PRIMARY KEY)'
...
describe('server/metadata middleware', () => {
  ...
  it('normalizeAzuraCast uses fallback for missing song fields', async () => {
    ...
    const raw = {
      ...
      is_playing: false,
      now_playing: {
        song: { id: '', text: '', artist: null, title: null, album: '', art: null },
        elapsed: 0,
        duration: 0,
      },
      history: [],
